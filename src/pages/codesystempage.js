...
export default function Codesystempage() {
  ...
  const [ favCodeSystem, setFavCodeSystem ] = useState('');
  ...
  const getFavoriteCodeSys = () => {
      const userId = getAuthToken()
      axios.get(`${url}code/favCodeSystem/${userId}`)
      .then((res) => {
          setFavCodeSystem(res.data);
          console.log(res.data[1].code[1])
      })
      .catch(error => console.error(`Error: ${error}`));
  }
        ...
        <div style={{ width: "60%", height: "200px", margin: "0px auto", padding: "60px 0px" }}>
            ...
            <Tabs defaultActiveKey='1' centered>
                ...
                <TabPane tab='我的收藏' key='2'>
                    <FavoriteCodeSys favCodeSystem={favCodeSystem} getFavoriteCodeSys={getFavoriteCodeSys} filterList={filterList} setFileIndex={setFileIndex} setFile={setFile} setDeteil={setDeteil} handleFavorite={handleFavorite} />
                </TabPane>
            </Tabs>
        </div>
